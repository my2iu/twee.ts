...
var tweeTsToTs = {
	...
	createBookSubTree : function(subtree, indents) {
		...
		for (var n in passages)
			code += indents + this.passageCodeName(passages[n]) + ' : <Passage>null,\n';
		for (var n in submodules)
			code += indents + this.passageCodeName(submodules[n]) + ' : {\n'
				+ this.createBookSubTree(subtree[submodules[n]], indents)
				+ indents + '},\n';

		code += indents + 'get _submodules() : PassageModule[] { return [';
		for (var n = 0; n < submodules.length; n++)
			code += 'this.' + this.passageCodeName(submodules[n]) + ', ';
		code += ']; },\n';
		code += indents + 'get _passages(): Passage[] { return [';
		for (var n = 0; n < passages.length; n++)
			code += 'this.' + this.passageCodeName(passages[n]) + ', ';
		code += ']; },\n';
	...
	fixNewLinesBeforeCode : function(text) {
		// When text is followed by a code passage, we will remove the last newline 
		// in the text unless there are two or more newlines there. This is to prevent
		// new paragraphs from being created when we have code on two or more lines in
		// the middle of a paragraph.
		var endWhitespaceMatch = /\s*$/g.exec(text);
		var lastWhitespace = endWhitespaceMatch[0];
		if (this.countNewLines(lastWhitespace) > 1)
			return text;
		var content = text.substring(0, endWhitespaceMatch.index);
		text = content + 
			lastWhitespace.replace(/\n([^\n]*)$/g, function(match, afterNewLine) { return afterNewLine; });
		return text;
	},
	...
	checkOutLinks : function(logger)
	{
		var hasBadLinks = false;
		for (var passage in this.passageOutLinks) 
		{
			var outLinks = this.passageOutLinks[passage];
			for (var n = 0; n < outLinks.length; n++)
			{
				var link = outLinks[n];
				link = tweeTsHelpers.canonicalizePassageName(passage, link);
				if (this.passageOutLinks[link] == null) 
				{
					logger("Unresolved link from " + passage + " to " + link);
					hasBadLinks = true;
				}
			}
		}
	...
	extractOutLinks : function(text, outlinkArray){
		var match;
		var regex = /\[\[(.*?)\]\]/g;
	...
	handlePassage : function(passageLine, passage, pathBase, outputElement) {
		...
		this.passageOutLinks[passageName] = [];
		...
		substituted += this.newLines(this.countNewLines(match[0]));
		...
		while (match != null) {
			...
			this.extractOutLinks(markdownText, this.passageOutLinks[passageName]);
			substituted += this.strEscape(this.fixNewLinesBeforeCode(markdownText));
		...
		var markdownText;
		if (!match)
			markdownText = passage.substring(pos);
		else
			markdownText = passage.substring(pos, match.index);
		this.extractOutLinks(markdownText, this.passageOutLinks[passageName]);
		substituted += this.strEscape(markdownText) + '");';
		substituted += this.newLines(this.countNewLines(match[1]) - 1);
...
var tweeTsHelpers = {
	...
	getBadLinksReport : function(logger) {
		return tweeTsToTs.checkOutLinks(logger);
	}
}
...
$(document).ready(function() {
	importer.onImportDone(function() {
		...
		if  ($.inArray('lib/twee-ts-runtime.ts', tsFiles) >= 0) {
			// Put the runtime.ts library at the beginning
			tsFiles.splice($.inArray('lib/twee-ts-runtime.ts', tsFiles), 1);
			tsFiles.unshift('lib/twee-ts-runtime.ts');
		}
		else if  ($.inArray('twee-ts-runtime.ts', tsFiles) >= 0) {
			// Put the runtime.ts library at the beginning
			tsFiles.splice($.inArray('twee-ts-runtime.ts', tsFiles), 1);
			tsFiles.unshift('twee-ts-runtime.ts');
		}
