...
var tweeTsToTs = {
	...
	outputBook : function(outputElement)
	{
		var code = 'var book = {\n';
		for (var n = 0; n < this.passageNames.length; n++)
		{
			var passage = this.passageNames[n];
			code += this.passageCodeName(passage) + ': Passage,\n';
		}
		code += '};\n';
	...
	handlePassage : function(passageLine, passage, outputElement) {
		...
		this.outputPassage(passageName, substituted, outputElement);
	},
	outputPassage(passageName, passageCode, outputElement) {
		var code =  '';
		...
		code += 'passages.push(new ' + className + '());';
