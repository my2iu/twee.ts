...
var tweeTsToTs = {
	...
	simplifyPassageName : function(namePath) {
		...
		// Remove all slashes at the beginning
	...
	handlePassage : function(passageLine, passage, outputElement) {
		...
		if (!match) return;
		var passageName = this.simplifyPassageName(match[1].trim());
		var tags = [];
		var rest = match[2];
		var tagRegex = /\s*\[([^\]]*)\](.*)/;
	...
	outputPassage(passageName, passageCode, tags, outputElement) {
		...
		code += 'passages.push(book';
		var path = passageName.split('/');
		for (var i = 0; i < path.length; i++) code += '.' + this.passageCodeName(path[i]);
		code += ' = new ' + className + '());';
